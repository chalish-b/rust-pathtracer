...
impl Camera {
    ...
    pub fn viewport_size(&self) -> Vec2 {
        ...
        // The viewport distance doesn't matter, it's just an arbitrary choice to
        // calculate the ray direction, which will be the same regardless of
        // distance. The FOV and the Aspect is the real way to calculate the
        // viewport size But to make things easier with defocus blur, we make it
        // the same as the focus plane, which actually needs a specific point
        // and a distance instead of just direction height = 2 * viewport_dist *
        // tan(v_fov / 2)
    ...
    pub fn axes(&self) -> (Vec3, Vec3, Vec3) {
        ...
        // In this case, normalization will fail, so we can  fallback to using
        // `Vec3::X`.
